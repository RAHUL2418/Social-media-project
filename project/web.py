...
import numpy as np
import pandas as pd
data = pd.read_csv(r"C:\Users\ragul\Downloads\project\instagram_reach (3).csv")
...
def PlotData(features):
    ...
    for mem in features:
        ...
        plt.style.use('seaborn-whitegrid')
        plt.grid(True)
        plt.title('likes according to'+ str(mem))
...
def PredictionsWithConstantFollowers(model, followerCount, scaller, maxVal):
    ...
    plt.ylabel('Likes')
    plt.title('your likes might be like this graph ')
    plt.show()
app = Flask(__name__,static_folder="static")

app.config['MYSQL_HOST'] = 'localhost'
app.config['MYSQL_USER'] = 'root'
app.config['MYSQL_PASSWORD'] = 'rahul123'
